...
class RAM(nn.Module):
  def __init__(
    ...
    if vit == 'swin_b':
      ...
      assert image_size == vision_config[
        'image_res']
    ...
    self.prompt_length = len(
      self.tokenizer(self.prompt).input_ids
    ) - 1
    ...
    q2l_config = BertConfig.from_json_file(
      f'{CONFIG_PATH}/configs/q2l_config.json'
    )
    ...
    self.tagging_head.resize_token_embeddings(
      len(self.tokenizer)
    )
    ...
    self.label_embed = nn.Parameter(
      torch.zeros(
        self.num_class,
        q2l_config.encoder_width
      )
    )

    if q2l_config.hidden_size != 512:
      self.wordvec_proj = nn.Linear(
        512,
    ...
    self.class_threshold = torch.ones(
      self.num_class
    ) * self.threshold
  ...
  def generate_tag(
    ...
    label_embed = torch.nn.functional.relu(
      self.wordvec_proj(self.label_embed)
    )

    image_embeds = self.image_proj(
    ...
    label_embed = label_embed.unsqueeze(0).repeat(
      bs,
      1,
      1
    )
    ...
    targets = torch.where(
      ...
      > self.class_threshold.to(image.device),
      torch.tensor(1.0).to(image.device),
      torch.zeros(self.num_class
                  ).to(image.device)
    )
    ...
    for b in range(bs):
      ...
      token_chinese = self.tag_list_chinese[
        index].squeeze(axis=1)
      tag_output_chinese.append(
        ' | '.join(token_chinese)
      )
